...
app.post('/api/login', (req, res) => {
    ...
    db.get(`SELECT * FROM Users WHERE username = ?`, [username], async (err, user) => {
        if (user && await bcrypt.compare(password, user.password)) {
            ...
            res.cookie('token', token, { httpOnly: true, path: '/', sameSite: 'strict' }).json({ success: true });
...
app.post('/api/collect', auth, (req, res) => {
    db.get(`SELECT income FROM Users WHERE id = ?`, [req.userId], (err, user) => {
        if(!user) return res.json({added:0});
...
app.post('/api/upgrade', auth, (req, res) => {
    ...
    db.get(`SELECT balance FROM Users WHERE id = ?`, [req.userId], (err, user) => {
        ...
        db.run(`UPDATE Users SET balance = balance - ?, income = income + ? WHERE id = ?`, [cost, addIncome, req.userId], () => {
            res.json({ success: true });
        });
    });
});
